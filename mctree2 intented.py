...
class MCtree():
    def __init__(self, network, komi=7.5, board_size=19, base_play_hist=['filler'], base_board_stat=[], use_num_thread=1, use_num_gpus=0):
        ...
        if self.base_board_stat == []: # Empty board
            ...
            relative_score = self.board.score(mode=1)
            ...
            if self.pass_enabled(V, relative_score, b_no_legal_verts):
                self.base_all_legal_positions_b.append(1)
            else:
                self.base_all_legal_positions_b.append(-32767)
            if self.pass_enabled(V, -relative_score, w_no_legal_verts):
                self.base_all_legal_positions_w.append(1)
            else:
...
# utility
    ...
    def pass_enabled(self, nneval, relative_score, legal_verts_count):
        # depends on the board stat at the time
        if self.dumbpass_enabled: return True

        relative_score_winning = False
        if nneval > 0.8: # nested if to prevent scoring all the time
            if relative_score > 0.:
                relative_score_winning = True
        if legal_verts_count < 20 or relative_score_winning: 
            return True
...
# main functions
    ...
    def gen_value(self, branch):
        # will become obsolete after eval_batch is implemented
...
# node: [trunk_name, played move, board_stat, edge probability(tensor), list of every legal action, value, action vector(Q), visit vector(N)]
    ...
    #         with tf.device('/CPU:0'):
    #             outputs = self.network.predict(dense_input)

    #     for i in range(self.batch_cycle):
    #         Q, N = edge[1], edge[0]
    #         V = outputs[i]
    #         edge = self.edge_dict[self.batched_positions[i]]
    #         action_vector = self.edge_dict[self.batched_positions[i]][6]
    #         edge[1] = (Q*N+V) / (N+1) # change total action value ***************
    #         action_vector = self.assign_value_to_tensor(action_vector, edge[1], index) #***************
    #     self.batch_cycle = 1
    #     self.batched_positions = []
    ...
    def initialize_children(self, node):
        ...
        # trunk_name, P,  = node[0] + node[5], node[3], 
        # for index, value in enumerate(legal_moves):
        #     if value == 1:
        #         selection_probability = P[index]
        #         connects_to = self.num_to_pos(index)
        #         self.edge_dict[trunk_name + connects_to] = [0, 0, selection_probability, connects_to, trunk_name, len(trunk_name)/2]
...
# node: [trunk_name, played move, board_stat, edge probability(tensor), list of every legal action, value, action vector(Q), visit vector(N)]
    ...
    def create_new_node(self, trunk_name, move_played):
        ...
        if self.base_to_play == 'B':
            to_play = 'B' if no_of_moves_played_from_base % 2 == 0 else 'W'
        else:
            to_play = 'B' if no_of_moves_played_from_base % 2 == 1 else 'W'
        ...
        P, V = self.gen_value(branch)
        P, V = P[0], V[0][0]

        if len(branch) < 4: #legal moves not accessed from parents doesn't have the indicator if pass is enabled
            if self.pass_enabled(V, self.board.score(mode=1), 361-legal_moves.count(0)): #Handling the last legal move, namely pass
                legal_moves.append(1)
            else:
                legal_moves.append(-32767)
        empty = tf.constant([0. for i in range(362)])
        new_node = [trunk_name, move_played, self.board.board_stat, P, legal_moves, V, empty, empty]
        ...
        for i in range(int(len(branch)/2)): # access all ancestors, yes, ancestors XD
            ...
            action_vector, visit_vector = node[6], node[7]
            Q, N = float(action_vector[index]), float(visit_vector[index])
            Q = (Q * N + V) / (N + 1)
            N = N + 1
            node[6] = self.assign_value_to_tensor(action_vector, Q, index) #SUBJECT TO BE CHANGED ******************************************
            node[7] = self.assign_value_to_tensor(visit_vector, N, index)

        if branch != '':
            self.base_stat[5] += 1
        self.initialize_children(new_node)
